...
func (c *categoryRepository) DeleteCategory(ctx context.Context, id uint) error {
	...
	default:
		...
		if len(modelCategory.Products) > 0 {
			log.Errorf("[CategoryRepository] DeleteCategory - 3: %v", "Category has products")
...
func (c *categoryRepository) GetAllCategories(ctx context.Context, page int, limit int, search string, sortBy string, sortOrder string) ([]model.Category, int64, error) {
	...
	default:
		...
		if err := query.Preload("Products").Order(sortBy + " " + sortOrder).Limit(limit).Offset(offset).Find(&modelCategories).Error; err != nil {
			log.Errorf("[CategoryRepository] GetAllCategories - 2: %v", err)
			return nil, 0, err
		}
