...
func (u *userController) EditAssignUserToRole(c *fiber.Ctx) error {
	...
	userRoleID := conv.StringToUint(c.Params("userRoleID"))
...
func (u *userController) GetAllUserRoles(c *fiber.Ctx) error {
	...
	users, totalRecord, err := u.userUsecase.GetAllUserRoles(ctx, req.Page, req.Limit, req.Search, req.SortBy, req.SortOrder)
	if err != nil {
	...
	paginationInfo := pagination.CalculatePagination(req.Page, req.Limit, int(totalRecord))
...
func (u *userController) GetAllUsers(c *fiber.Ctx) error {
	...
	users, totalRecord, err := u.userUsecase.GetAllUsers(ctx, req.Page, req.Limit, req.Search, req.SortBy, req.SortOrder)
	if err != nil {
		log.Errorf("[UserController] GetAllUserRoles - 3: %v", err)
		return c.Status(fiber.StatusInternalServerError).JSON(fiber.Map{
			"message": err.Error(),
