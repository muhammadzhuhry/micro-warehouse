...
// ROLE: Create, Update, Delete, Get by id, Get all
...
func (r *roleRepository) CreateRole(ctx context.Context, role model.Role) error {
	select {
	case <- ctx.Done():
		log.Errorf("[RoleRepository] CreatedRole - 1: %v", ctx.Err())
		return ctx.Err()
	default:
	}
...
func (r *roleRepository) DeleteRole(ctx context.Context, id uint) error {
	...
	if len(modelRole.Users) > 0 {
		...
		return errors.New("role has assigned users, cannot delete")
	}
...
func (r *roleRepository) GetAllRoles(ctx context.Context) ([]model.Role, error) {
	...
	if err := r.db.WithContext(ctx).Preload("Users").Find(&modelRoles).Error; err != nil {
